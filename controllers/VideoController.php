...
class VideoController extends Controller {
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if (Yii::$app->request->post('save') == 'save') {
                ...
                $model->Status = Video::STATUS_PUBLISHED;
    ...
    function actionPublish($id) {
        ...
        if ($model->Status == Video::STATUS_SAVED OR $model->Status == Video::STATUS_UNPUBLISHED) {
            $model->Status = Video::STATUS_PUBLISHED;
            $model->DatePosted = Date('Y-m-d H:i:s', time());
            $model->save();
        }
    ...
    function actionUnpublish($id) {
        $model = Video::findOne($id);
        if ($model->Status == Video::STATUS_PUBLISHED) {
            $model->Status = Video::STATUS_UNPUBLISHED;
            $model->DatePosted = NULL;
            $model->save();
        }
        return $this->redirect(['view', 'id' => $model->Id]);
